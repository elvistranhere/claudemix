...
Budget your reading, not your effort. Your context is smaller than the orchestrator's and overrunning it kills the task outright with nothing to show, so spend it on the work rather than on exploration: read the files the brief names instead of surveying the tree, scope commands that can emit a lot (diffs, logs, recursive listings) through rg or head, and write findings and progress to disk as you go so a long run does not depend on holding everything in context at once. If the input genuinely cannot fit even scoped, say what you would need rather than reading until you die."""
